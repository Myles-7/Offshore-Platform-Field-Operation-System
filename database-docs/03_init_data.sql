...
-- ======================================================================
-- database-docs supplement: MVP third demo work order
-- Existing db/init_data.sql already contains ASSIGNED and COMPLETED orders.
-- This supplement adds one IN_PROGRESS order for PC and mobile list testing.
-- ======================================================================
...
INSERT INTO work_order (
  ...
  leader_id, maintainer_id, planned_start_time, planned_end_time,
  actual_start_time, actual_end_time, status, priority, acceptance_required,
  source_type, local_id, server_id, version, sync_status, device_id, operator_id, remark
) VALUES (
  'WO-20260604-003',
  (SELECT id FROM project_info WHERE project_code = 'PRJ-OFFSHORE-A-2026'),
  (SELECT id FROM work_order_template WHERE template_code = 'TPL-ANTICORROSION-001'),
...
INSERT INTO work_order_assignment (
  work_order_id, assigner_id, assignee_id, assignment_role, assignment_status,
  assigned_at, accepted_at, local_id, server_id, version, sync_status, device_id, operator_id, remark
) VALUES (
  (SELECT id FROM work_order WHERE work_order_no = 'WO-20260604-003'),
  (SELECT id FROM sys_user WHERE username = 'pm_zhang'),
  (SELECT id FROM sys_user WHERE username = 'worker_li'),
  'MAINTAINER',
  'ACCEPTED',
